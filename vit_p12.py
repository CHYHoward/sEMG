...
class ViT_FNet(nn.Module):
    def __init__(self, window_size, num_channel, F=5, P=12, Q=10, dim=144, depth=1, heads=8, mlp_dim = 720, dropout = 0.4, emb_dropout = 0.4, pool = 'cls', number_gesture=49, class_rest=False):
        super().__init__()
        assert pool in {'cls', 'mean'}, 'pool type must be either cls (cls token) or mean (mean pooling)'
